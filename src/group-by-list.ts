...
export function groupByList<X, K>(
    ...
    .accumulate<{
        ...
        switch (cmd.type) {
          case "insert": {
            ...
            if (groupIdList.size === 0) {
              // New group - use set
              mapCmds.push({ type: "set", key, value: List([rx.snapshot]) });
            } else {
          ...
          case "move": {
            ...
            const newGroupIdList = groupIdList.remove(oldGroupIndex).insert(newGroupIndex, id);
            groupLists.set(key, newGroupIdList);
      ...
      for (const upd of upds) {
        ...
        if (oldKey !== newKey) {
          ...
          if (oldKey !== undefined) {
            ...
            if (oldGroupIdList) {
              ...
              if (indexInOldGroup !== -1) {
                const newOldGroupIdList = oldGroupIdList.remove(indexInOldGroup);
                if (newOldGroupIdList.size === 0) {
                  ...
                  mapCmds.push({ type: "update", key: oldKey, command: removeListCmd });
                }
              }
            }
          }
          ...
          if (newGroupIdList.size === 0) {
            // New group - use set
            mapCmds.push({ type: "set", key: newKey, value: List([rx.snapshot]) });
          } else {
            // Existing group - use update with insert command
            const insertListCmd: ListCommand<X>[] = [
              { type: "insert", index: insertIndex, value: rx.snapshot },
            ];
            mapCmds.push({ type: "update", key: newKey, command: insertListCmd });
          }
          ...
          mapCmds.push({ type: "update", key: newKey, command: updateListCmd });
        }
      }
  ...
  return Reactive.create(graph, mapOps, allChanges, initialGroups);
}
