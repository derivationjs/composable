...
describe("List<Map<string, string>> with mapList and mapMap", () => {
  ...
  it("should map over list and nested maps", () => {
    ...
    const mapped = mapList(graph, mapOps, listWithData, (rxMap) => {
      return mapMap<string, string, string>(graph, stringOps, rxMap, (rxString) => {
        ...
        return Reactive.create(graph, stringOps, upperChanges, upper.value);
      });
    });
    graph.step();
  ...
  it("should handle inserting a new map into the list", () => {
    ...
    // Insert a new map
    changes.push([{ type: "insert", index: 1, value: IMap({ name: "bob", city: "nyc" }) }]);
    graph.step();
  ...
  it("should handle complex batch operations", () => {
    ...
    changes.push([
      ...
      { type: "update", index: 2, command: [{ type: "update", key: "role", command: "guest" }] as MapCommand<string, string>[] },
      { type: "remove", index: 1 },
    ]);
    graph.step();
  ...
  it("should call mapping functions correct number of times", () => {
    ...
    changes.push([{ type: "insert", index: 1, value: IMap({ x: "10", y: "20" }) }]);
    graph.step();
