...
describe("projectTuple", () => {
  ...
  it("should extract element 0 initial value", () => {
    type T = [Cell<number>, Cell<string>];
    ...
    const source = Reactive.create<Tuple<T>>(
      ...
      tupleOps,
      changes,
      Tuple(new Cell(10), new Cell("hello")),
    );

    const first = projectTuple(graph, source, 0);
    graph.step();

    expect(first.snapshot.value).toBe(10);
  });
  ...
  it("should extract element 1 initial value", () => {
    ...
    expect(second.snapshot.value).toBe("hello");
  });
  ...
  it("should propagate updates to the projected element", () => {
    ...
    expect(first.snapshot.value).toBe(42);
  });
  ...
  it("should work with map elements", () => {
    type T = [IMap<string, Cell<number>>, Cell<string>];
    ...
    const source = Reactive.create<Tuple<T>>(
      ...
      tupleOps,
      changes,
      Tuple(IMap({ a: new Cell(1), b: new Cell(2) }), new Cell("hello")),
    );

    const map = projectTuple(graph, source, 0);
    graph.step();

    expect(map.snapshot.get("a")?.value).toBe(1);
    expect(map.snapshot.get("b")?.value).toBe(2);

    // Update via inner map command
    changes.push([[{ type: "update", key: "a", command: 10 }], null]);
    graph.step();

    expect(map.snapshot.get("a")?.value).toBe(10);
    expect(map.snapshot.get("b")?.value).toBe(2);
  });
  ...
  it("should handle sequential updates across steps", () => {
    ...
    graph.step();
    expect(first.snapshot.value).toBe(20);

    changes.push([30, null]);
    graph.step();
    expect(first.snapshot.value).toBe(30);
  });
  ...
  it("should handle updates to both elements simultaneously", () => {
    ...
    expect(first.snapshot.value).toBe(42);
    expect(second.snapshot.value).toBe("world");
  });
});
